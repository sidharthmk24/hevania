    ...
    return (
        <section className="w-full max-w-full mx-auto px-6 md:px-12 lg:px-20 pb-30 flex flex-col md:flex-row gap-12 md:gap-24 font-sans">
            ...
            <div className="md:w-[40%] flex flex-col gap-6 pt-4">
                <h2 className="text-[2.5rem] md:text-[3.25rem] font-light text-[#555] leading-[1.1] tracking-tight">
            ...
            <div className="md:w-[60%] flex flex-col">
                    ...
                    {faqs.map((faq, index) => {
                        ...
                        return (
                            <div key={index} className="border-b border-gray-200">
                                ...
                                >
                                    <span className={`text-[1.05rem] font-medium transition-colors ${isOpen ? 'text-[#666]' : 'text-[#777] group-hover:text-[#444]'}`}>
                                        {faq.question}
                                    </span>
                                    <span className="ml-6 transition-transform duration-300 grid place-items-center">
                                        {isOpen ? (
                                            <ChevronUp strokeWidth={1.5} size={20} className="text-[#888]" />
                                        ) : (
                                            <ChevronDown strokeWidth={1.5} size={20} className="text-[#3BC9F5]" />
                                        )}
                                    </span>
                                </button>
                                {/* Expandable answer */}
                ...
                <div className="mt-10 flex justify-center">
                    <button className="text-[#888] hover:text-[#444] border-b border-[#888] hover:border-[#444] transition-colors pb-0.5 text-sm font-medium">
                        View More
                    </button>
                </div>
            </div>
